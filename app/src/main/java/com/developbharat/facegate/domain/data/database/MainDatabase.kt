...
fun RoomDatabase.searchFaceInFacesTable(
    vectors: Iterable<Float>, minThreshold: Float, maxThreshold: Float
): Person? {
    ...
    if (cursor.moveToFirst()) {
        ...
        if (colId == -1 || colName == -1 || colThumbnail == -1) {
        ...
        cursor.close()
        return if (l2norm in minThreshold..maxThreshold) {
            Person(id = id, name = name, thumbnail = thumbnail)
        } else {
            null
        }
    }

    // dispose cursor and raise exception in case we don't have any rows
    cursor.close()
